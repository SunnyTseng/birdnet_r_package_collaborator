...
##### Workflow that ecologists usually use? What kind of functionality that birdnetR package should provide?
...
##### Wrap around the BirdNET Analyzer python code instead of the python package?

1.  How about simply wrap around the BirdNET Analyzer instead of the python pacakge?
    -   to enhance the indepandency (to reduce the complexity of the structure)

    -   avoid the troubles of using `reticulate` package, which can be causing issues during the CRAN review

    -   already have successful example runing BirdNET using R ([NSNSDAcoustics](https://github.com/nationalparkservice/NSNSDAcoustics), [Elas's script](https://github.com/eliasb03/teamshrub_bowman_honours/blob/main/scripts/aru_analysis/01_birdNET_functions_v2.R))

    -   Maybe I missed the advantages of wrapping the python package using R? More robust? I have few expereince with Python (took one course long time before), but I have experience with the GUI, the python code following the instructions
